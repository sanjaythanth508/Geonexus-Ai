...
export default function PredictionReport() {
  ...
  let badgeColor = '#22D3EE', badgeBg = 'rgba(34,211,238,0.06)', badgeBorder = 'rgba(34,211,238,0.22)', categoryLabel = 'Good';
  if (scoreNum >= 70) { badgeColor = 'var(--c-success)'; badgeBg = 'rgba(16,185,129,0.06)'; badgeBorder = 'rgba(16,185,129,0.22)'; categoryLabel = 'Excellent'; }
  else if (scoreNum < 45) { badgeColor = '#F43F5E'; badgeBg = 'rgba(244,63,94,0.06)'; badgeBorder = 'rgba(244,63,94,0.22)'; categoryLabel = 'Poor'; }
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{
          ...
          <div className="glass-bright" style={{
            ...
            boxShadow: 'var(--shadow-xl)', overflow: 'hidden', padding: 'clamp(24px, 5vw, 40px)',
            ...
            <div style={{
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-display)', fontWeight: '900', fontSize: '24px', letterSpacing: '-0.02em' }}>
            ...
            <div style={{
              ...
              background: 'var(--text-muted)', border: '1px solid var(--border-subtle)',
              borderRadius: 'var(--r-lg)', padding: '20px', marginBottom: '32px'
            }}>
              <div>
                <span style={{ fontSize: '10.5px', fontWeight: '800', letterSpacing: '0.08em', color: 'var(--cyan)', textTransform: 'uppercase' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: badgeColor, color: '#000', fontWeight: '850', fontSize: '12px'
                }}>
                  {categoryLabel}
                </div>
              </div>
            ...
            <div style={{ marginBottom: '32px' }}>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: '850', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px', letterSpacing: '-0.01em' }}>
                <CheckCircleIcon /> Spatial Logistics & Water Infrastructure
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                <div style={{ padding: '16px', borderRadius: 'var(--r-md)', background: 'var(--c-surface)', border: '1px solid rgba(34,211,238,0.15)' }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'var(--cyan)', fontWeight: '700', margin: 0 }}>Corridor Logistics Bonus: +{(highway_corridor_bonus * 100).toFixed(0)}%</p>
                </div>
                <div style={{ padding: '16px', borderRadius: 'var(--r-md)', background: 'var(--c-surface)', border: '1px solid rgba(168,85,247,0.15)' }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'var(--purple)', fontWeight: '700', margin: 0 }}>Water Reliability Index: {(river_reliability_bonus * 100).toFixed(0)}%</p>
                </div>
              </div>
            </div>
            ...
            {sortedCriteria.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: '850', marginBottom: '14px', letterSpacing: '-0.01em' }}>
                ...
                <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--r-md)', overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }} className="responsive-table">
                    <thead>
                      <tr style={{ background: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', textAlign: 'left' }}>
                    ...
                    <tbody>
                      {sortedCriteria.map(([crit, v], idx) => (
                        <tr key={crit} style={{ borderBottom: idx < sortedCriteria.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                          ...
                          <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '750', color: 'var(--cyan)' }}>
            ...
            {!searchedSuggestion && !searchingSuggestion && (
              <div style={{
                ...
                background: 'var(--c-surface)', border: '1px solid rgba(56,189,248,0.22)',
                display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap'
              }}>
                <div>
                  <div style={{ fontSize: '11.5px', fontWeight: '800', color: 'var(--cyan)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                ...
                >
                  Suggest Nearest Better Location
                </button>
              </div>
            )}
            ...
            {searchingSuggestion && (
              <div style={{
                ...
                color: 'var(--cyan)', fontWeight: '750', fontSize: '13px'
              }}>
                <LoadSpinner /> Searching GIDC industrial corridors and substation proximity within 20 km...
              </div>
            )}
            ...
            {searchedSuggestion && suggestion && suggestion.mcda_final_suitability_score && (
              <div style={{
                marginTop: '28px', padding: '18px 20px', borderRadius: '16px',
                background: 'var(--c-surface)', border: '1px solid rgba(16,185,129,0.25)',
                display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap'
              }}>
                <div>
                  ...
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '600', marginTop: '2px' }}>
                    Alternative location ({suggestion.district || 'Gujarat'}) offers a higher suitability score of <b>{Number(suggestion.mcda_final_suitability_score).toFixed(1)}/100</b>.
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    className="btn-primary"
                    style={{ padding: '8px 16px', fontSize: '12.5px', background: 'var(--c-success)', border: 'none', boxShadow: 'var(--shadow-md)' }}
                  >
                    View Report
                  </button>
                  <button
                    ...
                    className="btn-ghost"
                    style={{ padding: '8px 16px', fontSize: '12.5px', color: 'var(--cyan)', borderColor: 'rgba(56,189,248,0.3)' }}
                  >
                    Compare
                  </button>
                </div>
