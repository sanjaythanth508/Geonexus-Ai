      ...
      {steps.map((step, i) => (
        <div key={i} className="step-item" style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: i < steps.length - 1 ? '1' : 'unset' }}>
          <div style={{
            ...
            color: i < current ? 'var(--c-success)' : i === current ? 'var(--c-accent-600)' : 'var(--text-muted)',
            fontWeight: i === current ? '800' : '600',
            fontSize: '13px', fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap',
          }}>
            <div className="step-dot" style={{
              width: '26px', height: '26px', borderRadius: '50%',
              background: i < current ? 'var(--c-success)' : i === current ? 'var(--c-accent-50)' : 'var(--c-neutral-50)',
              border: `2px solid ${i < current ? 'var(--c-success)' : i === current ? 'var(--c-accent-500)' : 'var(--border-subtle)'}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '11px', fontWeight: '900', flexShrink: 0,
              color: i < current ? 'var(--text-primary)' : i === current ? 'var(--c-accent-600)' : 'var(--text-muted)',
            }}>
              {i < current ? '' : i + 1}
            </div>
            {step}
...
function ScoreBadge({ score, color }) {
  ...
  const c = color || (score >= 70 ? 'var(--c-success)' : score >= 40 ? 'var(--c-warning)' : 'var(--c-error)');
  return (
    <div style={{
      display: 'inline-flex', alignItems: 'center', gap: '8px',
      padding: '8px 16px', borderRadius: '24px',
      background: 'var(--c-surface)', border: `1px solid ${c}`, color: c,
    }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        background: selected ? `var(--c-surface)` : disabled ? 'var(--c-surface-alt)' : 'var(--c-surface)',
        border: `1.5px solid ${selected ? color : disabled ? 'var(--border-subtle)' : 'var(--border-default)'}`,
        color: selected ? color : disabled ? 'var(--text-faint)' : 'var(--text-secondary)',
    ...
    >
      <div style={{
        ...
        background: selected ? color : 'var(--c-neutral-50)',
  ...
  return (
    <Layout>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', maxWidth: '1000px', margin: '0 auto', width: '100%' }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontWeight: '850', fontSize: '17px', letterSpacing: '-0.02em' }}>
            <span style={{ color: 'var(--c-accent-600)' }}>Industry</span>
            <span style={{ color: 'var(--text-primary)' }}> Compare</span>
          </span>
          {step >= 1 && loc && (
            <div style={{
              display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 12px',
              background: 'var(--c-accent-50)', border: '1px solid var(--c-accent-200)',
              borderRadius: '20px', fontSize: '12px', color: 'var(--c-accent-600)', fontWeight: '750',
      ...
      <main style={{ maxWidth: '1000px', width: '100%', margin: '0 auto', padding: 'clamp(24px,4vw,48px) clamp(16px,4vw,24px)' }} className="anim-fadeIn">
        ...
        {step === 0 && (
          <div className="card" style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid var(--border-default)' }}>
            <div style={{ padding: '24px clamp(16px, 4vw, 32px) 20px', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              {loc && <div style={{ marginTop: '10px', fontSize: '13px', color: 'var(--c-accent-600)', fontWeight: '700', fontVariantNumeric: 'tabular-nums' }}> Siting coordinates: {loc.lat.toFixed(5)}°N, {loc.lon.toFixed(5)}°E</div>}
            </div>
            <div style={{ height: '420px', position: 'relative' }}>
              <MapComponent
                onLocationSelect={handleLocSelect}
                markers={loc ? [{ lat: loc.lat, lon: loc.lon, color: 'var(--c-accent-500)', label: 'Selected Site' }] : []}
              />
            </div>
        ...
        {step === 1 && (
          <div className="card" style={{ borderRadius: '20px', padding: '36px clamp(16px, 4vw, 36px)', border: '1px solid var(--border-default)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '28px' }}>
              {/* Sector A */}
              <div>
                <h4 style={{ fontSize: '12px', fontWeight: '800', color: 'var(--c-primary-600)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '14px' }}>Industry Sector A</h4>
                <div style={{ display: 'grid', gap: '8px', gridTemplateColumns: '1fr 1fr' }}>
                  {allIndustries.map(ind => (
                    <IndustryCard
                      key={ind} ind={ind} color="var(--c-primary-600)"
              ...
              {/* Sector B */}
              <div>
                <h4 style={{ fontSize: '12px', fontWeight: '800', color: 'var(--c-accent-600)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '14px' }}>Industry Sector B</h4>
                <div style={{ display: 'grid', gap: '8px', gridTemplateColumns: '1fr 1fr' }}>
                  {allIndustries.map(ind => (
                    <IndustryCard
                      key={ind} ind={ind} color="var(--c-accent-600)"
        ...
        {/* Loading Spinner */}
        {loading && (
          <div className="card" style={{ borderRadius: '20px', border: '1px solid var(--border-default)', padding: '48px', textAlign: 'center' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '14.5px', fontWeight: '650' }}>Loading prediction...</p>
          </div>
        )}
        ...
        {step === 2 && resultA && resultB && !loading && (
          <div ref={pdfRef} className="anim-fadeIn">
            ...
            <div style={{ marginBottom: '24px', padding: '14px 18px', borderRadius: '12px', background: 'var(--c-surface-alt)', border: '1px solid var(--border-subtle)', fontSize: '13px', color: 'var(--text-muted)' }}>
              ...
              &nbsp;·&nbsp; Sector A: <strong style={{ color: 'var(--c-primary-600)' }}>{fmt(industryA)}</strong>
              &nbsp;·&nbsp; Sector B: <strong style={{ color: 'var(--c-accent-600)' }}>{fmt(industryB)}</strong>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '32px' }}>
              {[
                ...
                { label: fmt(industryB), result: resultB, color: 'var(--c-accent-500)', score: scoreB, isWinner: winner === 'B' },
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }} className="responsive-table">
                    ...
                    <tbody>
                      {criteriaKeys.map((key, idx) => {
                        ...
                        const delta = sA - sB;
                        const deltaColor = delta > 0 ? 'var(--c-primary-600)' : delta < 0 ? 'var(--c-accent-600)' : 'var(--text-muted)';
                        return (
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setStep(0); setLoc(null); setResultA(null); setResultB(null); setError(''); }} className="btn-ghost" style={{ padding: '12px 24px', color: 'var(--c-primary-600)', borderColor: 'var(--c-primary-200)' }}>
                New Comparison
              </button>
